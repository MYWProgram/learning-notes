...
```javascript
// 1 栈操作(先进后出)
push()		// 在数组末尾添加新元素;返回值是数组长度
pop() 		// 取出数组中的最后一个元素,修改length属性;返回值为删除的元素
// 2 队列操作(先进先出)
push()
shift()		// 取出数组中的第一个元素,修改length属性;返回值为删除的元素
unshift() 	// 在数组最前面插入项,返回值为数组的长度
// 3 排序方法
reverse()	// 翻转数组
sort(); 	// 即使是数组sort也是根据字符，从小到大排序
// 带参数的sort是如何实现的？
// 4 操作方法
concat()  	// arr1.concat(arr2),新数组接收拼接的参数,原来的数组不会改变
slice() 	// 从当前数组中截取一个新的数组，不影响原来的数组，参数start从0开始,end从1开始
splice()	// 删除或替换当前数组的某些项目，参数start, deleteCount, options(要替换的项目)
// 5 位置方法
indexOf()、lastIndexOf()   //如果没找到返回-1
// 6 迭代方法 不会修改原数组(可选)
every()		// 对数组中每一个元素进行判断,返回Boolean;传入一个函数,函数有三个参数
filter()	// 测试数组返回一个新数组,新数组元素为通过测试之后的新元素;传入一个函数,函数有三个参数
forEach()	// 遍历数组,相当于for循环;传入一个函数,函数有三个参数
map()		// 数组中每个元素都执行传入函数功能,返回的新数组包含执行之后的元素
some()
// 7 方法将数组的所有元素连接到一个字符串中,可以添加对应的分隔符
join()
```
...
// 2 字符串操作方法
concat()   		// 参数为需要拼接的字符串,用一个空串来接收
slice()    		// 第一个参数是开始截取的位置,第二个参数是停止截取的位置,停止的位置取不到
substring() 	// 第一个参数是开始截取的位置,第二个参数是停止截取的位置,停止的位置取不到
substr()   		// 第一个参数是开始截取的位置,第二个参数是截取的长度
// 3 位置方法
...
// 6 其它
search()
replace()		// 第一个参数为已有的,第二个参数是替换后的
split()			// 第一个参数是串中删掉的,第二个是保留的个数
fromCharCode()
